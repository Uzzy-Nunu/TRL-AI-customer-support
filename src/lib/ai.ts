...
import type { ConversationMessage, CreateSupportInput, AiSupportResponse } from "./types";
...
export async function generateSupportResponse(input: CreateSupportInput, history: ConversationMessage[] = []): Promise<{ response: AiSupportResponse; model: string; latencyMs: number }> {
  ...
  const conversation = history.map((message) => ({ role: message.role, content: redact(message.content) }));
  const request = JSON.stringify({ ...input, complaint: redact(input.complaint), name: redact(input.name), subject: redact(input.subject), conversation });
