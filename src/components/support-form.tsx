...
export function SupportForm() {
  ...
  const submit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    setState("loading"); setError(""); setFieldErrors({});
    try {
      ...
      if (!response.ok || data.error) {
        ...
        setFieldErrors(Object.fromEntries(Object.entries(data.error?.fields || {}).map(([key, value]) => [key, Array.isArray(value) ? String(value[0]) : String(value)])));
  ...
  if ((state === "success" || result) && result) return <section aria-labelledby="response-heading" className="rise rounded-3xl border border-[#cfdcc7] bg-[#f8fbf4] p-5 sm:p-8">
    <div className="flex items-center gap-3"><div className="grid h-10 w-10 place-items-center rounded-full bg-[#dfead9] text-xs font-bold text-[#4f6845]">AM</div><div><p className="font-semibold">Amara · The Racket Lifestyle</p><p className="text-xs text-[#70746d]">Just now</p></div><span className="ml-auto grid h-9 w-9 place-items-center rounded-full bg-[#d9f94a]"><Icon name="check" /></span></div>
    <h2 id="response-heading" ref={responseHeading} tabIndex={-1} className="sr-only">Reply from The Racket Lifestyle team</h2>
    <div className="mt-5 space-y-4">{(result.messages || [{ role: "customer" as const, content: form.complaint, createdAt: result.createdAt }, ...(result.aiResponse ? [{ role: "team" as const, content: result.aiResponse, createdAt: result.updatedAt }] : [])]).map((message, index) => message.role === "customer" ? <div key={`${message.createdAt}-${index}`} className="flex justify-end"><div className="max-w-[90%] rounded-2xl rounded-tr-sm bg-[#171916] px-4 py-3 text-sm leading-6 text-white"><p>{message.content}</p></div></div> : <div key={`${message.createdAt}-${index}`} className="flex items-start gap-3"><div className="grid h-8 w-8 shrink-0 place-items-center rounded-full bg-[#dfead9] text-[10px] font-bold text-[#4f6845]">AM</div><div className="max-w-[90%] rounded-2xl rounded-tl-sm border border-[#dfe8d9] bg-white px-4 py-3"><p className="mb-2 text-xs font-semibold text-[#4f6845]">Amara · The Racket Lifestyle</p><p className="whitespace-pre-line text-[15px] leading-7 text-[#31352e]">{message.content}</p></div></div>)}</div>
    ...
    <ConversationComposer request={result} onSent={(next) => { setResult(next); setState("success"); }} />
  </section>;
  return <form onSubmit={submit} noValidate aria-busy={state === "loading"} className="rounded-3xl border border-[#dedfd9] bg-white p-5 shadow-[0_16px_50px_rgba(24,32,20,.05)] sm:p-8">
    ...
    <div className="mt-7 grid gap-5 sm:grid-cols-2">
      <Field label="Your name" id="name" value={form.name} onChange={(v) => update("name", v)} placeholder="Alex Morgan" required error={fieldErrors.name} />
      <Field label="Email address" id="email" type="email" value={form.email} onChange={(v) => update("email", v)} placeholder="alex@example.com" required error={fieldErrors.email} />
      <div className="sm:col-span-2"><Field label="Subject" id="subject" value={form.subject} onChange={(v) => update("subject", v)} placeholder="What can we help with?" required error={fieldErrors.subject} /></div>
      ...
      <div className="sm:col-span-2"><label htmlFor="complaint" className="mb-2 block text-sm font-semibold">Your message <span className="font-normal text-[#a03d32]">*</span></label><textarea id="complaint" required minLength={10} maxLength={5000} value={form.complaint} onChange={(e) => update("complaint", e.target.value)} placeholder="Type your message here..." aria-describedby="complaint-help" className="focus-ring min-h-36 w-full resize-y rounded-2xl border border-[#d9ddd5] bg-[#fbfcfa] px-4 py-3 text-sm leading-6 placeholder:text-[#9a9d96]" />{fieldErrors.complaint && <p className="mt-2 text-xs text-[#a03d32]">{fieldErrors.complaint}</p>}<div id="complaint-help" className="mt-2 flex justify-between gap-3 text-xs text-[#858a80]"><span>Please don’t include passwords or payment credentials.</span><span>{form.complaint.length}/5000</span></div></div>
...
function Field({ label, id, value, onChange, placeholder, type = "text", required = false, optional = false, error }: { label: string; id: string; value: string; onChange: (value: string) => void; placeholder: string; type?: string; required?: boolean; optional?: boolean; error?: string }) {
  return <div><label htmlFor={id} className="mb-2 block text-sm font-semibold">{label} {required && <span className="font-normal text-[#a03d32]">*</span>}{optional && <span className="font-normal text-[#8a8e85]"> (optional)</span>}</label><input id={id} type={type} required={required} value={value} onChange={(e) => onChange(e.target.value)} placeholder={placeholder} aria-invalid={Boolean(error)} aria-describedby={error ? `${id}-error` : undefined} className="focus-ring min-h-12 w-full rounded-xl border border-[#d9ddd5] bg-[#fbfcfa] px-4 text-sm placeholder:text-[#9a9d96]" />{error && <p id={`${id}-error`} className="mt-2 text-xs text-[#a03d32]">{error}</p>}</div>;
}
