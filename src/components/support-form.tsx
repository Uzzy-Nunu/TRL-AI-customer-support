...
export function SupportForm() {
  ...
  if (state === "success" && result) return <section aria-labelledby="response-heading" className="rise rounded-3xl border border-[#cfdcc7] bg-[#f8fbf4] p-6 sm:p-9">
    <div className="flex items-start justify-between gap-4"><div><p className="eyebrow">Your support request</p><h2 id="response-heading" ref={responseHeading} tabIndex={-1} className="mt-2 text-2xl font-semibold tracking-tight">Here’s what we found</h2></div><span className="grid h-10 w-10 shrink-0 place-items-center rounded-full bg-[#d9f94a]"><Icon name="check" /></span></div>
    <div className="mt-6 rounded-2xl border border-[#dfe8d9] bg-white p-5"><div className="flex flex-wrap items-center gap-3"><StatusBadge status={result.status} /><span className="rounded-full bg-[#eff2ed] px-2.5 py-1 text-[11px] font-bold text-[#5d6259]">{result.category}</span><UrgencyBadge urgency={result.urgency} /></div><p className="mt-5 whitespace-pre-line text-[15px] leading-7 text-[#31352e]">{result.aiResponse}</p><div className="mt-5 border-t border-[#e5e8e2] pt-4"><p className="eyebrow">Next step</p><p className="mt-1 text-sm leading-6 text-[#50564d]">{result.safeNextStep}</p></div></div>
    {result.needsEscalation && <div className="mt-4 flex gap-3 rounded-xl border border-[#e6d7bb] bg-[#fffaf0] p-4 text-sm leading-6 text-[#765a28]"><Icon name="alert" size={19} /><p><strong>Our support team should take a closer look.</strong> We’ve flagged this request for escalation.</p></div>}
    <div className="mt-6 flex flex-wrap gap-3"><Link href={`/dashboard/${result.id}`} className="focus-ring inline-flex min-h-11 items-center justify-center rounded-full bg-[#171916] px-5 text-sm font-semibold text-white hover:bg-[#3f4936]">View request <span className="ml-2"><Icon name="arrow" size={15} /></span></Link><button className="focus-ring min-h-11 rounded-full border border-[#cdd4c7] px-5 text-sm font-semibold hover:bg-white" onClick={() => { setForm(initial); setResult(null); setState("idle"); }}>Ask another question</button></div>
  </section>;
  return <form onSubmit={submit} noValidate aria-busy={state === "loading"} className="rounded-3xl border border-[#dedfd9] bg-white p-5 shadow-[0_16px_50px_rgba(24,32,20,.05)] sm:p-8">
    <div className="flex items-start justify-between gap-4"><div><p className="eyebrow">Start a conversation</p><h2 className="mt-2 text-2xl font-semibold tracking-tight">Tell us what’s on your mind</h2></div><span className="hidden rounded-full bg-[#eef3e8] px-3 py-1 text-xs font-semibold text-[#4f6845] sm:block">Usually replies in a moment</span></div>
    <div className="mt-7 grid gap-5 sm:grid-cols-2">
      ...
      <div className="sm:col-span-2"><label htmlFor="complaint" className="mb-2 block text-sm font-semibold">How can we help? <span className="font-normal text-[#a03d32]">*</span></label><textarea id="complaint" required minLength={10} maxLength={5000} value={form.complaint} onChange={(e) => update("complaint", e.target.value)} placeholder="Share a few details and we’ll point you in the right direction." aria-describedby="complaint-help" className="focus-ring min-h-36 w-full resize-y rounded-xl border border-[#d9ddd5] bg-[#fbfcfa] px-4 py-3 text-sm leading-6 placeholder:text-[#9a9d96]" /><div id="complaint-help" className="mt-2 flex justify-between gap-3 text-xs text-[#858a80]"><span>Please don’t include passwords or payment credentials.</span><span>{form.complaint.length}/5000</span></div></div>
    ...
    <div aria-live="polite" className="mt-5">{state === "loading" && <p className="flex items-center gap-2 text-sm font-medium text-[#4f6845]"><span className="pulse-dot h-2 w-2 rounded-full bg-[#6b893e]" />Checking that for you…</p>}{state === "error" && <div role="alert" className="flex items-start gap-2 rounded-xl border border-[#edceca] bg-[#fff5f3] p-3 text-sm leading-6 text-[#923d34]"><Icon name="alert" size={18} /><span>{error}{result ? " Your request is saved and can be retried from My requests." : ""}</span></div>}</div>
